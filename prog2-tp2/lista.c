...
lista *lista_pesquisa_substring(lista *lst, char *substring) {
	int i;
	l_elemento *posicao = lst->inicio;
	lista *lista_iguais;
	lista_iguais = lista_nova();
	for (i = 0; i < lst->tamanho; i++) {
		if (strstr(posicao->str, substring) != NULL) {
			lista_insere(lista_iguais, posicao->str, NULL);
		}
...
void lista_imprime(lista *lista_sem) {
	int j;
	l_elemento *posicao = lista_sem->inicio;
	for(j = 0; j < lista_sem->tamanho; j++) {
		printf("Pos %d: %s", j+1, posicao->str);
		posicao = posicao->proximo;
	}
}
...
lista *lista_concatena(lista *lst1, lista *lst2) {
	...
	int i;
	l_elemento *posicao;
	nova_lista = lista_nova();
	posicao = lst1->inicio;
	for (i = 0; i < lst1->tamanho; i++) {
		lista_insere(nova_lista, posicao->str, NULL);
		posicao = posicao->proximo;
	}
	posicao = lst2->inicio;
	for (i = 0; i < lst2->tamanho; i++) {
		lista_insere(nova_lista, posicao->str, NULL);
		posicao = posicao->proximo;
	}
	return nova_lista;
