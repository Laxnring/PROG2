...
{
    elemento *carta_remover, *percorrer;
    int i = 0;
    carta_remover = lista_inicio(baralho);
    for (percorrer = ordem->inicio; percorrer != NULL; percorrer = percorrer->proximo) {
        if (!strcmp(carta_remover->str, percorrer->str)) {
            carta_remover = lista_remove(baralho, carta_remover);    
            return i; 
        }
        i++;
    }
...
{
    char *max;
    max = malloc(50*sizeof(char));
    int aux, i, maximo=0;

    for (i = 0; i < jogadores->tamanho; i++) {
        aux = tira_carta(baralho, ordem);
        printf("%d\n", aux);
        if (aux > maximo) {
            maximo = aux;
            strcpy(max, vetor_elemento(jogadores, i));
        }
    }
            printf("\n");
    return max;
...
int numero_vitorias(vetor *vencedores, const char *nome)
{
    ...
    for (i = 0; i < vencedores->tamanho; i++) {
        if (!strcmp(vencedores->elementos[i].str, nome)) {
            contador++;
        }
    }
    return contador;
